...
4. Open `src/config.ts` and paste both endpoints:
   ...
   formspree: {
     ...
     booking: "https://formspree.io/f/YOUR_BOOKING_ID",
   },
   ```
...
2. Navigate to `public_html/` (or the folder tied to `sound-ready.com`).
3. Upload **the contents of `dist/`** (not the folder itself) — so `index.html` sits directly in `public_html/`.
